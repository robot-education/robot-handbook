...
class TitleSequence():
    ...
    """
    def __init__(self, add_numbers: bool = True, default_color: style.Color = style.DEFAULT_COLOR) -> None:
    ...
    def next(self, title: str, color: style.Color | None = None) -> Animation:
        ...
        if (self._number == 2):
            self._first = text
            return Write(self._first, run_time=style.Time.MEDIUM)
        else:
            return Transform(self._first, text, run_time=style.Time.MEDIUM)
